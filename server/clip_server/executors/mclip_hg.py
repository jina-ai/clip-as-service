...
import numpy as np
import torch
from transformers import CLIPFeatureExtractor, CLIPModel, PreTrainedModel, PretrainedConfig, AutoTokenizer, AutoModel
...
class CLIPEncoder(Executor):
    def __init__(
        self,
        pretrained_model_name_or_path: str = 'M-CLIP/XLM-Roberta-Large-Vit-B-32',
        ...
        **kwargs,
        ...
        self._vision_preprocessor = CLIPFeatureExtractor.from_pretrained(
            base_feature_extractor or clip_model_name_or_path
        )
        self._tokenizer = AutoTokenizer.from_pretrained(
            base_tokenizer_model or pretrained_model_name_or_path
        )

        self._model = MultilingualCLIP.from_pretrained(pretrained_model_name_or_path)
        self._clip_model = CLIPModel.from_pretrained(clip_model_name_or_path)
                ...
                for minibatch, batch_data in _img_da.map_batch(
                    ...
                    with self.monitor(
                        ...
                        minibatch.embeddings = (
                            self._clip_model.get_image_features(**batch_data)
                            .cpu()
                            .numpy()
                            .astype(np.float32)
                        )
            ...
            if _txt_da:
                for minibatch, batch_data in _txt_da.map_batch(
                    ...
                    with self.monitor(
                        ...
                        minibatch.embeddings = (
                            self._model.forward(batch_data)
                            .cpu()
                            .numpy()
                            .astype(np.float32)
                        )
